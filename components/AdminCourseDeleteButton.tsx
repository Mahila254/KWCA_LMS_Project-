...
type AdminCourseDeleteButtonProps = {
  slug: string;
  title: string;
};

export default function AdminCourseDeleteButton({
  slug,
  title,
}: AdminCourseDeleteButtonProps) {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (!response.ok) {
        alert(data.error || "Something went wrong while deleting the course.");
        return;
      }
